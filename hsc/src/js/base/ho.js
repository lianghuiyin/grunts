...
HOJS.lib = {
    ...
    valiPasswordValue:function(valiValue){
        var reg = /^\w{5,15}$/;//长度在6~16之间，只能包含字符、数字和下划线
        return this.testReg(reg,valiValue);
    },
    /**
    ...
    */
    ...
    /**
     * [dateDiffForWorkday 计算两个日期间的工作日天数，排除周六及周日]
     * @param  {[datetime]} date1 [起始时间]
     * @param  {[datetime]} date2 [结束时间]
     * @param  {[array]} holidays [日期枚举，数组中所有日期将被排除]
     * @return {[number]}   [输出秒值]
     */
    dateDiffForWorkday: function(date1, date2, isSkipWeekend, holidays) {
        if(!date1 || !date2){
            //日期不能为空
            return;
        }
        ...
        d2.setHours(dt1.getHours());
        d2.setMinutes(dt1.getMinutes());
        d2.setSeconds(dt1.getSeconds());
        // 小时、分、秒设置为相等值，然后计算两者的工作日天数偏差
        var dayDiff = this.dayDiffForWorkday(d1, d2, isSkipWeekend, holidays);
        // 年、月、日设置为相等值，然后计算两者的小时、分、秒部分的偏差
        d1 = new Date(dt1.getTime());
        d2 = new Date(dt1.getTime());
        d2.setHours(dt2.getHours());
        d2.setMinutes(dt2.getMinutes());
        d2.setSeconds(dt2.getSeconds());
        var timeOffset = this.dateDiff("S",d1,d2);
        return 24 * dayDiff * 60 * 60 + timeOffset;
    },
    /**
     * [dayDiffForWorkday 计算两个日期间的工作日天数，排除周六及周日]
     * @param  {[datetime]} date1 [起始时间]
     * @param  {[datetime]} date2 [结束时间]
     * @param  {[array]} holidays [日期枚举，数组中所有日期将被排除]
     * @return {[number]}   [输出天数值]
     */
    dayDiffForWorkday: function(date1, date2, isSkipWeekend, holidays) {
        //计算两个日期间的工作日天数，排除周六及周日
        if(!date1 || !date2){
            //日期不能为空
            return;
        }
        if(isSkipWeekend == undefined){
            //默认跨过周末
            isSkipWeekend = true;
        }
        //holidays功能有待扩展，扩展起来也比较容易，
        //先算出holidays中有多少个元素在date1与date2范围内，
        //然后把目前的算法得到的结果减去算出的元素个数就行。
        var dt1 = (date1 instanceof Date) ? date1 : new Date(Date.parse(date1.replace(/-/g, '/')));
        var dt2 = (date2 instanceof Date) ? date2 : new Date(Date.parse(date2.replace(/-/g, '/')));
        if(!isSkipWeekend){
            return this.dateDiff('D',dt1,dt2);
        }
        var startDay = dt1.getDay(),
            endDay = dt2.getDay();
        var startPartDays = 7 - startDay,
            endPartDays = 7 - endDay;
        dt1.addDays(startPartDays);
        dt2.addDays(endPartDays);
        var splitDays = this.dateDiff('D',dt1,dt2);
        splitDays = splitDays * 5/7;
        var remainderSplitDays = 0;
        if(startPartDays == 1){
            remainderSplitDays += 1;
        }
        if(endPartDays == 1){
            remainderSplitDays -= 1;
        }
        splitDays += remainderSplitDays;
        return splitDays + startPartDays - endPartDays;
    },
    /**
     * [addDaysForWorkday 计算从开始日期增加若干个工作日后结束日期是哪天，排除周六及周日]
     * @param {[type]} startDate [description]
     * @param {[type]} count     [description]
     * @param {[type]} holidays  [description]
     * @return {[date]}   [输出结束日期]
     */
    addDaysForWorkday: function(startDate, count, isSkipWeekend, holidays) {
        //计算从开始日期增加若干个工作日后结束日期是哪天，排除周六及周日
        if(!startDate){
            //日期不能为空
            return;
        }
        ...
        //holidays功能有待扩展，扩展起来也比较容易，
        //先算出holidays中有多少个元素在startDate与dt2范围内，
        //然后把目前的算法得到的结果（即dt2），加上算出的元素个数就行。
        var dt1 = (startDate instanceof Date) ? startDate : new Date(Date.parse(startDate.replace(/-/g, '/')));
        if(!count){
            return dt1;
        }
        var dotValue = HOJS.lib.accSub(count,Math.floor(count));//小数部分值，比如0.5表示半个工作日
        ...
        if(count){
            if(isSkipWeekend){
                ...
                // }
                var startPartDays = 7 - startDay;
                var startPartDays2 = startPartDays - 2;//减去周末的天数
                if(count <= startPartDays2){
                    dt1.addDays(count);
                }
                else{
                    dt1.addDays(startPartDays);
                    if(startPartDays2 > 0){
                        //当dt1为周六或周日时，startPartDays2为-1、-2，这时不需要对count做减法
                        //当dt1为周五时，startPartDays2为0，这时，对count做减法没有意义
                        count -= startPartDays2;
                    }

                    //与5相除得到的余数，当count<5时，remainderCount将等于count，后面count - remainderCount将为0，所以不影响后面的逻辑
                    remainderCount = count % 5;

                    dt1.addDays((count - remainderCount) * 7/5);
                    if(remainderCount === 0){
                        dt1.addDays(-2);
                    }
                    else{
                        dt1.addDays(remainderCount);
                    }
                }
            }
            else{
        ...
        //后面逻辑是处理小数值部分，支持半个工作日这样的算法
        // if(dotValue > 0){
        //     var newHours = HOJS.lib.accMul(8,dotValue);//小数点后部分值代表的小时数，8小时工作制
        //     if(dt1.getHours() <= 12){
        //         //中午1点前，则直接加上小数点后部分值代表的小时数，8小时工作制
        //         dt1.addHours(newHours);
        //     }
        //     else{
        //         //中午1点后，则从第二天8点开始算，而且要避开双休日
        //         if(isSkipWeekend){
        //             var curDay = dt1.getDay();
        //             if(curDay < 5){
        //                 dt1.addDays(1);
        //             }
        //             else{
        ...
        if(dotValue > 0){
            var newHours = HOJS.lib.accMul(24,dotValue);//小数点后部分值代表的小时数，24小时等分
            dt1.addHours(newHours);
        }
